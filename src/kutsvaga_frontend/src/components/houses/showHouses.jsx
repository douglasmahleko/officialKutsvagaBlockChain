...
class ShowHouses extends React.Component{
  ...
  componentDidMount(){
        ...
        if(this.state.userLevel ==="AGENT"){
          ...
          if(messages.length === 0){
            ...
            if(dta.length > 0){
              ...
              this.props.handleCloseSnackBar()
              this.props.handleCloseBar()
              this.props.handleClickSnackBar("Error occured", 'error')
            }
          } 
        }
      } catch (error) {
    ...
    headerShow(){
      return(
        <Box sx={{marginLeft:"5%"}}>
            <Typography variant="h3" gutterBottom> Available Houses </Typography>
      ...
      return(
        <Box>
            <div className={ !this.state.displayRearchResult ? "show" : "hide"}>
              <Grid >
                <SearchBar searchValue={this.state.search} onChange={(e) => this.setState({search:e.target.value})} 
                  placeholder="Search for a House" title={this.headerShow()} onClick={this.handleSearch} paid={this.props.paid} /> 
              </Grid>
              { this.state.data != null && this.state.tableHeader != null ? ( <CreateTable link2="agentEmail" handleCloseBar={this.props.handleCloseBar}
                  ...
                  data={this.state.data} tableHeader={this.state.tableHeader} handleCloseSnackBar={this.props.handleCloseSnackBar}
                  /> ) : (
                    <Skelet />
                  ) }
            </div>
            <div className={ this.state.displayRearchResult ? "show" : "hide"}>
              { this.headerShow() }
              { this.state.dataSearch.length > 0 && this.state.tableHeader != null ? ( <CreateTable link2="agentEmail" handleCloseSnackBar={this.props.handleCloseSnackBar}
                    handleClickSnackBar={this.props.handleClickSnackBar} getLevel={this.state.userLevel} paid={this.props.paid}
                    user={this.props.user} focus="MoreOnHouse" routie2="/moreURL" backendActor={this.props.backendActor} handleCloseBar={this.props.handleCloseBar}
