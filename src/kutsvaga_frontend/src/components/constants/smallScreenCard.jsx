...
function SmallScreenCard({note, setopenNum, logout, title, paid, runShortList,handleCloseBar, loading, setnumRoomsToTake, setopenNumForRooms, openNumForRooms, numRoomsToTake, numRooms, header, focus, moreOfDataNoTHead, openSetNumRooms, link3, shortList, setnumRooms, openNum, returnToMarket, takeHouse, onClick, link, getLevel, link2, routie, routie2, tableHead, tableHeaderForMore, user, moreOfData}){ 
        ...
        <Paper sx={{margin:'1%', marginLeft :{ xs:'150px'}}}>
            <Card elevation={1}>
                                ...
                                <TableBody>
                                    ...
                                    <TableRow>
                                    {
                                                paid ? (
                                                        ( focus === "MoreOnHouse") ? (
                                                        <div>
                                                            <Button onClick={() => moreOfData(tableHead, note, "More On A House")} variant="contained" size="small" color="primary" mt={5} > Details{/*More On House*/} </Button>
                                                            {
                                                                getLevel === 'CLIENT' ? (
                                                        ...
                                                        (focus === "housesByAgent") ? (
                                                            ...
                                                            (focus === "MoreOnClient") ? (
                                                                ...
                                                                (focus === "shortList") ? (
                                                                    ...
                                                                    getLevel === "CLIENT" ? (
                                                                            <LoadingButton onClick={() => takeHouse(user.email, note.address)} 
                                                                                variant="contained" size="small"  color="primary" mt={5} loading={loading}
                                                                                loadingPosition="end" >Confirm</LoadingButton>
                                                                        ) : (
                                                                            <Button onClick={() => moreOfData(tableHead, note, "More On ShortList")} variant="contained" size="small" color="primary" mt={5} > Details{/* More On ShortList*/}</Button>
                                                                        )
                                                                ) : (
                                                                    (focus === "Requirements") ? (
                                                                        <div>
                                                                            <Button onClick={() => onClick()}  variant="contained" size="small" color="primary" mt={5} >Recommend</Button>
                                                                            <Button onClick={() => moreOfData(tableHead, note, "More On Prerequisities")} sx={{marginLeft:"3px"}} variant="outlined" size="small" color="primary" mt={5} > Details{/* View Requirement*/}</Button>
                                                                        </div>
                                                                    ) : (
                                                                        (focus === "Recommend") ? (
                                                                            <div>
                                                                                <LoadingButton onClick={onClick}  variant="contained" size="small" color="primary" mt={5} loading={loading}
                                                                                  loadingPosition="end" endIcon={<RecommendIcon />} >Select</LoadingButton>
                                                                                <Button onClick={() => moreOfData(tableHeaderForMore, note, "More On A House")} sx={{marginLeft:"3px"}} variant="outlined" size="small" color="primary" mt={5} > Details{/*  More On House*/}</Button>
                                                                            </div>
                                                                        ) : (
                                                                            (focus === "Recommended") ? (
                                                                                <div>
                                                                                    ...
                                                                                    <Button onClick={() => moreOfDataNoTHead("house", note.address, "More On Reccommended House")} sx={{marginLeft:"3px"}} variant="outlined" size="small" color="primary" mt={5} > Details{/*  More On House*/}</Button>
                                                                                </div>
                                                                            ) : (
                ...
                <AddNum numRooms={numRooms} openNum={openNum} returnToMarket={returnToMarket}
                        setnumRooms={setnumRooms} setopenNum={setopenNum} title="Add rooms free" />
