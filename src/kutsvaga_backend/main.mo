...
actor {
  ...
  public type BitcoinActor = actor {
    get_balance : shared Text -> async Nat64;
  ...
  public shared query func getMyAutoReccomended(userEmail :Text) : async Result.Result<[Recomendations.Recomendation], Text> {
    ...
    for ((key, value) in recommendationsAuto.entries()) {
      if(value.agentEmail == userEmail){
        if(determineHouse(value.address)){
  ...
  public shared({caller}) func updateUser(args : Users.UserUpdateUnOfficial) : async (Text) {
  ...
  public shared({caller}) func setExpiry() : async (Bool) {
    changePaymentValidity(caller, false);
  };
  func changePaymentValidity(caller: Principal, value : Bool) : (Bool) {
  ...
  public shared({caller}) func returnToMarket(address : Text, client : Text, freeRooms: Int) : async (Text) {
    var unique = Text.concat(address, client);
    let toMarket = housesOffMarkets.remove(unique);
    let user_house : Houses.HouseOffMarket = Option.get(toMarket, default_offMarket);
    housesOffMarketsHistory.put(unique, user_house);
...
public shared query func getSpecificRecomendations(userEmail :Text) : async Result.Result<[Recomendations.Recomendation], Text> {
  ...
  public shared({caller}) func recomend(args : Recomendations.UnOfficialRecomendation) : async (Text) {
    ...
    if(Int.less(count, 3)){
      if(exist){
        ...
        if(determineHouse(args.address)){
          ...
          return "House is off the market now";
          }
        }
      }
      else{
    ...
    switch (clientNeeds.get(caller)) {
      ...
      case (?data) {
        ...
        if(recommendations.size() > 0){
          for((key, value1) in recommendations.entries()){
            if(unique == key){
              exist := true;
            };
          };
  ...
  func determineHouse(address:Text) : (Bool) {
  ...
  public shared({caller}) func confirm(address : Text) : async Text {
    ...
    for ((key, value) in shortListedHouse.entries()) {
      ...
      if(changePaymentValidity(caller, false)){
  ...
  public shared func get_p2pkh_address() : async (Text)  {
    await basic_bitcoin.get_p2pkh_address();
  }; 
  public shared func get_balance(addres : Text) : async (Nat64)  {
    await basic_bitcoin.get_balance(addres);
  }; 
  public shared func send(sendInfo : SendRequest) : async (Text)  {
    await basic_bitcoin.send(sendInfo);
  };
};